...
def insert_annotated(genome_dir):
    try:
        params = config()
        conn = psycopg2.connect(host="abcd",database="abcd",user="admin",password="12345",port = "1")
        cur = conn.cursor()
        # Début de transaction
        # cur.execute(requete)
        genome_attribute = parse_genome(genome_dir)
        # table (genome_sequence, length, chromosome, species_name, strain)
        insert_genome_query = f"INSERT INTO genome VALUES ({genome_attribute[0]}, {genome_attribute[1]}, {genome_attribute[2]}, {genome_attribute[3]}, {genome_attribute[4]}) RETURNING id_genome;"
        ...
        for seq_att in sequence_attribute_list:
            ...
            insert_sequence_query = f"INSERT INTO sequence VALUES ({seq_att[0]}, {seq_att[1]}, {seq_att[2]}, {seq_att[3]}, {seq_att[4]},{seq_att[5]}, {seq_att[6]}, {id_genome});"
        ...
        for annot_att in annotation_attribute_list:
            ...
            insert_sequence_query = f"INSERT INTO sequence VALUES ({annot_att[0]}, {annot_att[1]}, {annot_att[2]}, {annot_att[3]}, {annot_att[4]}, {annot_att[5]}, {annot_att[6]});"
            cur.execute(insert_sequence_query)
        # commit changes
        conn.commit()
