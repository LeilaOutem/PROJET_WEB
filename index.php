...
if(isset($_POST['login']))
{
   if(isset($_POST['email']) && isset($_POST['password']))
   {
      ...
      if($email !== "" && $passwd !== "")
      {
		  ...
		  $result = mysqli_query($_SESSION['db'],$query) or die(mysql_error());
		  $rows = mysqli_num_rows($result);
		  if($rows==1){
			//changing date
			$querydate = "UPDATE users SET date_last_connexion = CURDATE() WHERE email = '$email'";
			$newdate = mysqli_query($_SESSION['db'],$querydate) or die(mysql_error());
		...
		if(!empty($email) AND !empty($firstname) AND !empty($surname) AND !empty($passwd) AND !empty($confpassword) AND !empty($role))
		{
			if ($passwd == $confpassword) {
				...
				if (filter_var($email, FILTER_VALIDATE_EMAIL))
				{
					...
					$check_email = $_SESSION['db']->query('SELECT surname FROM users WHERE email="'.$email.'"');
					
					if(!is_null($check_email) and $check_email !== TRUE)
					{
						if (TRUE == $_SESSION['db']->query("INSERT INTO `users`(email, password, first_name, surname, phone_number, date_last_connexion, id_role) 
						VALUES ('$email','".hash('sha256', $passwd)."','$firstname','$surname','$phone_number',CURDATE(), (SELECT id_role FROM `role` WHERE descriptor = '$role'))")) {
						} else {
